...
import { editorThinkingStyle, PolishedEditor, WrappedPolishedEditor } from "./ui";
...
const MODEL_INFO_WIDGET_KEY = "zentui-model-info";
...
type EditorInstallMode = "none" | "standalone" | "wrapper" | "foreign";
...
export default function (pi: ExtensionAPI) {
	...
	// ── Model-info widget (used when a foreign editor like pi-vimmode owns the input) ──

	const installModelInfoWidget = (ctx: ExtensionContext): void => {
		if (!isTuiContext(ctx) || typeof ctx.ui.setWidget !== "function") return;
		ctx.ui.setWidget(
			MODEL_INFO_WIDGET_KEY,
			(_tui: TUI, theme: Theme): Component => ({
				render(width: number): string[] {
					...
					const parts = [model, provider].filter(Boolean).join(
						safeThemeFg(theme, "borderMuted", "  "),
					);
					...
					if (tl && tl !== "off") {
						metaParts.push(
							renderStyleForSourceOrFallback(
								theme,
								colorSource,
								editorThinkingStyle(config, tl),
								"muted",
								tl,
							),
						);
					}
					const line = ` ${metaParts.filter(Boolean).join(safeThemeFg(theme, "border", "  "))} `;
					return [line];
				},
				invalidate(): void {},
			}),
	...
	const uninstallModelInfoWidget = (ctx: ExtensionContext): void => {
		if (!isTuiContext(ctx) || typeof ctx.ui.setWidget !== "function") return;
		ctx.ui.setWidget(MODEL_INFO_WIDGET_KEY, undefined);
	};
	...
	const installEditor = (ctx: ExtensionContext): boolean => {
		...
		// If another extension (e.g., pi-vimmode) owns the editor, do NOT wrap it.
		// Wrapping third-party editors causes layout conflicts (model meta line
		// overlaps with autocomplete popups). Instead, show model info as a
		// thin widget bar above the editor.
		if (currentFactory && !isZentuiEditorFactory(currentFactory)) {
			uninstallPrototypePatches();
			installModelInfoWidget(ctx);
			wrappedEditorFactory = currentFactory;
			installedEditorFactory = currentFactory;
			editorInstallMode = "foreign";
			editorInstalled = true;
			return true;
		}
	...
	const uninstallEditor = (ctx: ExtensionContext): boolean => {
		const currentFactory = ctx.ui.getEditorComponent();
		if (currentFactory && !isZentuiEditorFactory(currentFactory) && editorInstallMode !== "foreign")
			return false;

		if (editorInstallMode === "foreign") {
			uninstallModelInfoWidget(ctx);
			wrappedEditorFactory = undefined;
	...
	const applyConfiguredUi = (ctx: ExtensionContext): ApplyUiResult => {
		...
		if (currentConfig.features.editor) {
			const currentFactory = ctx.ui.getEditorComponent();
			// In "foreign" mode the editor is owned by another extension; only
			// re-install if the foreign editor itself changed or the widget is missing.
			if (editorInstallMode === "foreign") {
				if (currentFactory && currentFactory !== installedEditorFactory) {
					// Foreign editor changed — re-install widget over the new editor.
					uninstallModelInfoWidget(ctx);
					installModelInfoWidget(ctx);
					wrappedEditorFactory = currentFactory;
					installedEditorFactory = currentFactory;
	...
	pi.on("agent_start", (event, ctx) => {
		...
		// Reconcile editor: another extension (e.g., pi-vimmode with async config
		// loading) may have replaced the editor component after session_start.
		// In "foreign" mode re-apply the model-info widget if the editor changed.
		// In normal modes re-wrap with the polished frame.
		if (isTuiContext(ctx) && currentConfig.features.editor) {
			const currentFactory = ctx.ui.getEditorComponent();
			if (currentFactory && currentFactory !== installedEditorFactory) {
				applyConfiguredUi(ctx);
				refresh();
